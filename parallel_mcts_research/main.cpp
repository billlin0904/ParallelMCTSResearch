...
struct Encoder {
	...
	static std::string Turn(const GomokuGameMove& move, const GomokuGameState& state, int32_t room_id, int32_t round_id) {
		Document document;
...
class GomokuRoom {
public:
	explicit GomokuRoom(int32_t room_id) 
		: room_id_(room_id)
		, round_id_(GetRoundID()) {
	}

	std::string EnterRoom() {
		return Encoder::EnterRoom(*state_, room_id_, round_id_);
	}

	std::string NewRound() {
		ai_ = std::make_unique<MCTS<GomokuGameState, GomokuGameMove>>();
		state_ = std::make_unique<GomokuGameState>();
		round_id_ = GetRoundID();
	...
	std::string Turn() {		
		...
		return Encoder::Turn(move, *state_, room_id_, round_id_);
	}
...
private:
	...
	void NewRoom(const std::shared_ptr<websocket::Session>& s) {
		...
		auto room = std::make_unique<GomokuRoom>(room_id);
		room->NewRound();
		s->Send(room->EnterRoom());
...
class GomokuGameClient : public websocket::WebSocketCallback {
public:
	GomokuGameClient()
		: room_id_(0)
		, round_id_(0) {
...
private:
	void NewRoundOrEnterRoom(std::shared_ptr<websocket::WebSocket> s, CommandID cmd) {
		...
		state_->ApplyMove(move);
		s->Send(Encoder::Turn(move, *state_, room_id_, round_id_));
	}
	...
	void Turn(std::shared_ptr<websocket::WebSocket> s, const Value &packet) {
		...
		s->Send(Encoder::Turn(search_move, *state_, room_id_, round_id_));
	}
...
int main() {
	...
	auto ws = websocket::WebSocket::MakeSocket("ws", "127.0.0.1", "9090", ios, new GomokuGameClient());
	ws->Connect();*/
	server.Run();	
