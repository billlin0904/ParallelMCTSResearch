  ...
  class dispatcher {
   private:
    ...
    int handle(char *message) {
      ...
      if (message == flot_buffer_ && strlen(flot_buffer_) == buffer_size_ - 1)
        return ERANGE;
      flot_buffer_ = message;
      return 0;
    }

    // Handle the case when strerror_r is not available.
    int handle(internal::null<>) {
      return fallback(strerror_s(flot_buffer_, buffer_size_, error_code_));
    }

    // Fallback to strerror_s when strerror_r is not available.
    int fallback(int result) {
      // If the buffer is full then the message is probably truncated.
      return result == 0 && strlen(flot_buffer_) == buffer_size_ - 1 ?
            ERANGE : result;
    }
    ...
    int fallback(internal::null<>) {
      errno = 0;
      flot_buffer_ = strerror(error_code_);
      return errno;
    }

   public:
    dispatcher(int err_code, char *&buf, std::size_t buf_size)
      : error_code_(err_code), flot_buffer_(buf), buffer_size_(buf_size) {}

    int run() {
      return handle(strerror_r(error_code_, flot_buffer_, buffer_size_));
    }
  };
...
FMT_FUNC internal::utf8_to_utf16::utf8_to_utf16(string_view s) {
  ...
  flot_buffer_.resize(length + 1);
  length = MultiByteToWideChar(
    CP_UTF8, MB_ERR_INVALID_CHARS, s.data(), s_size, &flot_buffer_[0], length);
  if (length == 0)
    FMT_THROW(windows_error(GetLastError(), ERROR_MSG));
  flot_buffer_[length] = 0;
}
...
FMT_FUNC int internal::utf16_to_utf8::convert(wstring_view s) {
  ...
  length = WideCharToMultiByte(
    CP_UTF8, 0, s.data(), s_size, &flot_buffer_[0], length, FMT_NULL, FMT_NULL);
  if (length == 0)
    return GetLastError();
  flot_buffer_[length] = 0;
  return 0;
